...
// Shorebird 的私有 dart-sdk 提供 Shorebird_ReadLinkHeader；我们的没有，
// 按规则 3 就地实现。解析的是 .vmcode 容器头，格式由我们的 linker
// (tools/linker.py) 定义、并经 spikes/b_route_phase2_groundtruth 实测反推：
//
//   offset 0            uint32 LE  N（映射条数）
//   offset 4            N × 8 字节 (uint32 sim_offset, uint32 cpu_offset)
//   ... 补零至页对齐（至少 4 页 = 16384）
//   之后                内嵌的 patch ELF
//
// 返回内嵌 ELF 的起始偏移。
int FhpReadLinkHeader(const uint8_t* data, size_t size) {
  ...
  const size_t content = sizeof(uint32_t) + static_cast<size_t>(count) * 8u;
  size_t pages = (content + kPageSize - 1) / kPageSize;
  if (pages < kMinPages) {
    pages = kMinPages;
  }
  const size_t offset = pages * kPageSize;
  if (offset >= size) {
    FML_LOG(ERROR) << "vmcode link header (" << offset
                   << " bytes) does not fit in file of " << size << " bytes";
    return 0;
  }
...
std::shared_ptr<PatchCacheEntry> PatchCacheEntry::Create(
  ...
  int elf_file_offset = FhpReadLinkHeader(elf_mapping->GetMapping(),
                                                 elf_mapping->GetSize());

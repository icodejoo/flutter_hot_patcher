...
| # | 需求 | 对应 REVIEW | 说明 |
|---|---|---|---|
| **R1 CanonicalName 对齐** | 从 Kernel `.dill` 取 库URI→类→成员 唯一路径对齐新旧版本函数；**实例级**（含 decl_line/column 区分同名闭包/重载） | S3、命门 | 替代源文件路径代理与无序多重集；混淆/文件移动/版本升级天然免疫（Kernel 名与 ELF 名解耦）。**spike 级探索已证伪一条捷径**（`r2_pool_probe/NOTES.md`）：`gen_snapshot --disassemble` 的函数名仍是 `file:///...` 绝对路径，跟 DWARF 方案同源，不会白给库 URI。**2026-07-31 已跑通并正面验证**（`r1_kernel_dill_probe/NOTES.md`）：`package:kernel` 的 `loadComponentFromBinary`（SDK 自带、现成公开 API，不必自己写二进制格式解析器）读出的 `Library.importUri` 对 `lib/` 下被 `package:` 引用的库文件是真正的 `package:xxx/yyy.dart`，**跨越完全不同的构建目录逐字节稳定**（实测：整个包目录搬到另一绝对路径重新编译，CanonicalName 不变），直接解决 COVERAGE_GAPS #19 part-file-moved 残留问题。诚实边界：入口脚本本身仍是 file:// URI；`fileOffset` 需换算才等价于 decl_line/column；接入 diff_linker 仍是独立的、更大的工程（这轮只验证 CanonicalName 来源）。**2026-07-31：Mac 已实现 `kernel_linker v1`**（`tools/kernel_linker/`，Dart 工具，代码在 Mac 本地未推送）：`FunctionId`（`libraryUri::Class.member`）用 `Library.importUri` 构造，跟这边独立验证的方案一致，在真实语料上 PASS——R1 从"验证过来源可行"进展到"生产实现已跑通" |
| **R2 对象池精确比对** | 解析快照对象池，逐 slot 比对两版同一 canonical slot 的常量内容 | S1 | 修"改 String/double/const 只动池、指令不变"的整类漏判；注意别做成新漏报源。**spike 级探索已验证可行机制**（`r2_pool_probe/NOTES.md`）：非 product `gen_snapshot --disassemble --code_comments` 会吐出独立的 `ObjectPool len:N` 完整列表，含每个 slot 的原始内容（数值常量给 IEEE754 位模式）；实测 `const fee=0.07→0.08` 场景——调用指令逐字节不变（复现 S1 漏判)，但各自 build 自己的池列表在同一 slot 显示 1.07→1.08，解码正确。**不必逆向对象池二进制布局**，复用 VM 自带诊断输出即可。**2026-07-31：Mac 的 kernel_linker v1 用了一条更简单的等效路径**——不解析对象池，而是在 Kernel IR 层对函数体做**规范化文本指纹**（`Printer.writeProcedureInLibrary` 剥离 `@offset`），常量值在 Kernel AST 里本来就是明文字面量（还没被编译进对象池），指纹直接就能看到 `0.05→0.07` 这类改动。实测 PASS，比对象池方案更早一层拦截、不需要 AOT 快照 |
| **R3 调用边完备提取** | 边提取包含：目标**身份**（并入条件1）、二级/unchecked 入口按地址区间归属、尾调 jmp、以及**间接/虚调/闭包/tear-off/池介导**调用的显式边界契约 | S2、S4、S5、B1、B2 | 静态侧对每个不可静态解析的转移点，要么建保守边、要么产出"运行时必须重定向此点"的义务清单并逐条核验。**spike 级审计**（`r3_boundary_contract/NOTES.md`，2026-07-31）：diff_linker 故意不为虚调用/闭包调用建传播边，这是**对的**——Gate1 V2 实测虚调用/静态直调都是"单点控制"（改一处、全类/全调用点统一生效），不需要标记调用方。真正缺口不在 diff_linker 静态模型，见下方新增 R3.1。**2026-07-31：kernel_linker v1 实现后发现一个新的红线**——它在 Kernel IR 层只追踪 `StaticInvocation` 节点（Dart 源码语法上的静态调用：顶层函数/`static`方法），**看不到 AOT 编译器的去虚化**（单态实例方法调用 `obj.method()`，Kernel 层是 `MethodInvocation`，被 AOT 的 CHA 分析编译成真正的直接跳转）——去虚化是 AOT 阶段的决定，Kernel IR 结构上就不知道。旧版机器码层 `diff_linker.py` 天然捕获这类调用（`v6_v7_v8/NOTES.md` "V6 去虚化路由"已验证 PASS），换到 Kernel 层之后这个安全网消失了，是 Kernel-IR 架构选择带来的新盲区，不是实现疏漏。建议：保守做法——任何 `MethodInvocation`/`InstanceInvocation` 引用了被改动函数的调用方一律播种为"可能受影响"（哪怕过报，不能漏报），精确做法（Kernel 层近似 CHA 判断单态性）工作量大且需跟 AOT 编译器逻辑同步，作为后续优化 |
...
| **R4 ICF/去重感知** | 从 Kernel 层做 ICF 感知对齐；被折叠函数不得丢 key；`removed`/折叠导致 key 消失时保守播种其调用方或硬失败 | S6、S7 | 修"被改函数折叠进 removed、从不播种、无告警"红线。**2026-07-31 设计讨论**：ICF（`ProgramVisitor::DedupInstructions`，源码确认 `FLAG_dedup_instructions` 默认 `true`，比对对象是编译出的 `Instructions`）发生在 `gen_snapshot` 阶段，严格晚于 Kernel IR，kernel_linker 结构上看不到折叠决定。两个选项：A（保守，fingerprint 相同的函数一律播种，可能过报）vs B（Kernel 层不处理，留给 AOT 层后处理）。**建议 A**，理由与项目"完备性优先于精确度"原则一致。**尝试用 `analyze_snapshot` 验证 Function→Code 映射能否直接给出 ICF ground truth（第三条路）——实测未能复现折叠**（两个字节应相同的玩具函数指向了不同 Code 对象，原因未查清，可能与 `analyze_snapshot` 只能读非 product 快照有关），这条路有依据但未验证，不能替代 Option A |
...
| **R9 规模** | 反向边索引 + worklist，线性传播；支撑数万函数真实 app | D4 | spike 的 O(轮×指令) 纯 Python 不可用于真实规模。**2026-07-31：Mac 的 kernel_linker v1 已实现**——反向调用图 + BFS worklist 传播，Dart 原生实现，替代旧 Python 工具的性能瓶颈 |
...
1. **前置 go/no-go：iOS 真机 W^X 复验（Gate1 阶段B）——已于 2026-07-31 通过**（详见
   `GATE1_REPORT.md` §14）：V1（改代码页）在 iOS 上确认双重封死（W^X errno=13 +
   MAP_JIT 也不能改已签名段），但 V2（纯数据重定向）在 iOS 真机上完全 PASS，静态
   直调链只靠 V2 边界重定向同样 PASS——V1 从不是运行时必需机制，"V1 不通"不威胁
   架构健全性。这道曾被认为"唯一可能整体推翻方案"的门已经过了。
2. **生产 linker 正式开建**：Mac 已实现 `kernel_linker v1`（R1 CanonicalName + R2
   常量指纹 + R3 部分调用图 + R9 规模化传播，PASS），按本规格继续做 R3 去虚化盲区
   （新发现，见上）+ R3.1（闭包枚举）+ R4-R8。

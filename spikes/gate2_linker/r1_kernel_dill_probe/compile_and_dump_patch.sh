...
"$AOT_RUNTIME" "$GEN_KERNEL" --target vm -Ddart.vm.product=true \
  ...
  --output "$HERE/probe_patch.dill" \
  "$PKG/bin/main.dart"
echo "compiled patch"

cd "$HERE"
echo "== BASE =="
"$SDK_OUT/dart" --packages=.dart_tool/package_config.json dump_canonical_names.dart probe.dill
echo "== PATCH =="
"$SDK_OUT/dart" --packages=.dart_tool/package_config.json dump_canonical_names.dart probe_patch.dill

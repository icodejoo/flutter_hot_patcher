...
## 7. 追加（2026-07-31）：V1 在 iOS 上确认不通，但已证明不是解释器路线的必需机制

Gate1 阶段B（iOS 真机复验）已开始，Mac 反馈 V1（本文 §1 表格里的"改代码页"那一手，
`mprotect` 改写既有调用指令）在 iOS 上确认被拦截——完全符合本文 §2.2/§3.1 的预判
（"几乎注定是不能"）。**但同一时间，`GATE1_REPORT.md` 新增的 V6 用例
（`gate1_mixed_execution/cases/v6_multihop_no_v1/NOTES.md`）在桌面 x64 上实测证明：
V1 从来不是解释器路线运行时依赖的机制**——一条多跳纯静态直调链，只靠传递闭包（把整条链
标记为转解释）+ 一次 V2 式数据字段重定向，完全不碰任何调用指令，也能正确生效。Mac 在
iOS 上用等价思路独立得出同一结论，两边交叉印证。

这不改变本文 §3.2/§4 对**机器码路线**（新逻辑必须是新机器码，iOS 上被 W^X+签名墙彻底
挡死）的结论——那个结论跟 V1 是否运行时必需无关，是"能不能在 iOS 引入新可执行代码"这个
更底层的平台限制。这条追加只是澄清：本文 §1"Dart AOT 里改指针/改调用指令这一半已被
Gate1 V1/V2 证实可行"这句话里的 V1 部分，对**解释器路线**而言，其物理改写机器码的手法
从未被生产架构实际依赖过，"V1 在 iOS 不通"不威胁解释器路线的健全性——真正决定 iOS 可行性
的始终是 V2（纯数据写）本身，这个结论没有变化。

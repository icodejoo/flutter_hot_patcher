...
// arm64 port of ../../cases/v4_gc_inside_patch — same scenario (interpreted
// patch allocates enough garbage to trigger real GC mid-execution; caller
// holds a heap-allocated marker across the call). Only the call-site-rewrite
// mechanism differs (arm64 bl + icache flush; see
// ../v1_replace_existing_function/host/main.dart).
//
// V4 补丁内触发 GC 在 Android arm64 上的移植版——同样的场景(解释执行的补丁
// 分配足够垃圾在执行期间触发真实 GC；调用方持有一个堆上标记对象跨调用存活)。
// 唯一不同的是调用点改写机制(arm64 bl + icache 刷新；见
// ../v1_replace_existing_function/host/main.dart)。
library;
...
void main(List<String> args) {
  ...
  if (!activated) {
    print('GATE1B-V4 INCONCLUSIVE: patch activation failed');
    exit(3);
  }
  ...
  if (patchedOk && markerOk) {
    print('GATE1B-V4-ANDROID-ARM64 PASS: GC triggered inside interpreted code did not corrupt caller roots or post-GC allocation on real arm64 hardware');
    exit(0);
  }
  print('GATE1B-V4-ANDROID-ARM64 FAIL: patchedOk=$patchedOk markerOk=$markerOk');
  exit(1);
}
...
///
/// arm64 调用点改写机制(单个调用点)。完整解释见
/// ../v1_replace_existing_function/host/main.dart。
///
/// args: [0]=bytecode path, [1]=own snapshot path, [2]=g static addr (hex),
/// [3]=f static addr (hex), [4]=fAlt static addr (hex).
